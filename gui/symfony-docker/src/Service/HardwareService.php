    ...
    public function startReader(): JsonResponse
    {
        try {
            ...
            if ($e->getCode() === 403) {
                ...
                return new JsonResponse('Une erreur s\'est produite lors de la requête HTTP (client error).', Response::HTTP_BAD_REQUEST);
            }
        } catch (ServerException $e) {
            // Pour les erreurs serveur
            $this->logger->error('Erreur serveur lors de la requête HTTP : ' . $e->getMessage(), ['exception' => $e]);
            return new JsonResponse('Une erreur s\'est produite lors de la requête HTTP (server error).', Response::HTTP_INTERNAL_SERVER_ERROR);
        } catch (NetworkException $e) {
            // Pour les erreurs de réseau
            $this->logger->error('Erreur réseau lors de la requête HTTP : ' . $e->getMessage(), ['exception' => $e]);
            return new JsonResponse('Une erreur réseau s\'est produite lors de la requête HTTP.', Response::HTTP_SERVICE_UNAVAILABLE);
        } catch (\Exception $e) {
